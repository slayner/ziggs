...
async def _init_global_schema(db):
    """Schema do registro global: servidores ativados + nodes exportados entre guilds."""
    await db.execute('''
    ...
    ''')
    # Nodes compartilhados entre guilds não-premium.
    # canon_export_id aponta para o ID canônico quando há duplicatas do mesmo node.
    await db.execute('''
        CREATE TABLE IF NOT EXISTS node_exports (
            ...
            scout_id        INTEGER,
            canon_export_id INTEGER,
            created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    ''')
    await db.execute(
        'CREATE INDEX IF NOT EXISTS idx_node_exports_spawn ON node_exports(spawn_timestamp)'
    )
    await db.commit()
...
async def _init_schema(db):
        ...
        for migration_sql in [
            ...
            # Nodes: guild premium (nodes nunca exportados) + % do node exportado pro banco
            "ALTER TABLE economy_config ADD COLUMN is_premium INTEGER DEFAULT 0",
            "ALTER TABLE economy_config ADD COLUMN exported_node_percent INTEGER DEFAULT 0",
            # Registro: se auto-cadastrado pelo jogador + status de membro da guild no jogo
            "ALTER TABLE registrations ADD COLUMN self_registered INTEGER DEFAULT 0",
            "ALTER TABLE registrations ADD COLUMN guild_member INTEGER DEFAULT 1",
        ]:
            try:
                await db.execute(migration_sql)
                await db.commit()
...
async def get_all_registrations_by_nick(nick: str) -> list[dict]:
    """Retorna TODOS os registros Discord associados a um nick (case-insensitive).
    Pode retornar múltiplos se o mesmo nick foi re-registrado para usuários diferentes."""
    nick = (nick or '').strip()
    if not nick:
        return []
    ...
    async with _db() as db:
        ...
        # Aliases primeiro
        cur = await db.execute(
            'SELECT user_id FROM registration_aliases WHERE LOWER(nick) = LOWER(?)', (nick,)
        )
        for row in await cur.fetchall():
            uid = row[0]
            cur2 = await db.execute('SELECT * FROM registrations WHERE user_id = ?', (uid,))
            r = await cur2.fetchone()
            if r:
                results[uid] = dict(r)
        # Registro principal (pode ter nick sem alias correspondente)
        cur = await db.execute(
            'SELECT * FROM registrations WHERE LOWER(nick) = LOWER(?)', (nick,)
        )
        for row in await cur.fetchall():
            uid = row['user_id']
            if uid not in results:
                results[uid] = dict(row)
    return list(results.values())


async def register_self(user_id: int, nick: str) -> None:
    """Registra o próprio jogador (auto-cadastro). Marca self_registered=1."""
    from datetime import datetime, timezone
    now = datetime.now(timezone.utc).isoformat()
    async with _db() as db:
        await db.execute(
            '''INSERT INTO registrations (user_id, nick, registered_by, registered_at, updated_at, self_registered, guild_member)
               VALUES (?, ?, ?, ?, ?, 1, 1)
               ON CONFLICT(user_id) DO UPDATE SET
                   nick = excluded.nick,
                   updated_at = excluded.updated_at,
            ...
            await db.execute(
                '''INSERT INTO registration_aliases (nick, user_id, registered_by, registered_at, updated_at)
                   ...
                   ON CONFLICT(nick) DO UPDATE SET
                       user_id = excluded.user_id,
                       updated_at = excluded.updated_at''',
                (nick, user_id, user_id, now, now),
            )
        except Exception:
            pass
        try:
            await db.execute(
                'UPDATE cta_battle_players SET user_id = ? WHERE LOWER(player) = LOWER(?)',
                (user_id, nick)
            )
        except Exception:
            pass
...
async def get_self_registered_users() -> list[dict]:
    """Retorna todos os jogadores que se auto-cadastraram (para verificação periódica de guilda)."""
    async with _db() as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            'SELECT user_id, nick, guild_member FROM registrations WHERE self_registered = 1'
        )
        rows = await cur.fetchall()
    return [dict(r) for r in rows]


async def update_guild_membership(user_id: int, is_member: bool) -> None:
    """Atualiza o status de membro da guild no jogo para um auto-cadastrado."""
    async with _db() as db:
        await db.execute(
            'UPDATE registrations SET guild_member = ? WHERE user_id = ?',
            (1 if is_member else 0, user_id),
        )
        await db.commit()


async def get_scout_paid_count(user_id: int, since_days: int = 30) -> int:
    """Conta quantos eventos o usuário recebeu pagamento como scout nos últimos `since_days` dias.
    Usado para verificar confiabilidade do scout antes de exportar um node."""
    from datetime import datetime, timezone, timedelta
    cutoff = (datetime.now(timezone.utc) - timedelta(days=since_days)).isoformat()
    try:
        async with _db() as db:
            cur = await db.execute(
                '''SELECT COUNT(*) FROM cta_split_payouts
                   WHERE kind = 'scout' AND user_id = ? AND created_at >= ?''',
                (user_id, cutoff),
            )
            row = await cur.fetchone()
        return int(row[0]) if row else 0
...
# ==================== NODE EXPORTS (GLOBAL — registry.db) ====================

async def export_node_global(
    source_guild_id: int,
    node_type: str,
    map_name: str,
    spawn_timestamp: int,
    ...
    """Exporta um node para o registro global. Detecta duplicatas (mesmo tipo+mapa
    em janela de ±15 min): se já existe, mantém o canônico e registra o novo como
    duplicata. Retorna o ID do registro canônico, ou None em caso de erro."""
    DEDUP_WINDOW = 900  # 15 minutos
    low = spawn_timestamp - DEDUP_WINDOW
    high = spawn_timestamp + DEDUP_WINDOW
    try:
        async with _global_db() as db:
            # Procura canônico existente (mesma guild, tipo, mapa, janela de tempo)
            cur = await db.execute(
                '''SELECT id, canon_export_id FROM node_exports
                   WHERE source_guild_id = ? AND node_type = ? AND map_name = ?
                     AND spawn_timestamp BETWEEN ? AND ?
                   ORDER BY id ASC LIMIT 1''',
                (source_guild_id, node_type, map_name, low, high),
            )
            existing = await cur.fetchone()
            if existing:
                canon_id = existing[1] or existing[0]
                return canon_id

            # Procura canônico de OUTRA guild (dedup cross-guild)
            cur = await db.execute(
                '''SELECT id FROM node_exports
                   WHERE node_type = ? AND map_name = ? AND spawn_timestamp BETWEEN ? AND ?
                     AND canon_export_id IS NULL
                   ORDER BY id ASC LIMIT 1''',
                (node_type, map_name, low, high),
            )
            cross = await cur.fetchone()

            if cross:
                # Já existe um canônico de outra guild — registra como duplicata
                canon_id = cross[0]
                await db.execute(
                    '''INSERT INTO node_exports
                       (source_guild_id, node_type, map_name, spawn_timestamp,
                        scout_name, scout_id, canon_export_id)
                       VALUES (?, ?, ?, ?, ?, ?, ?)''',
                    (source_guild_id, node_type, map_name, spawn_timestamp,
                     scout_name, scout_id, canon_id),
                )
            else:
                # Primeiro registro deste node — é o canônico
                cur = await db.execute(
                    '''INSERT INTO node_exports
                       (source_guild_id, node_type, map_name, spawn_timestamp,
                        scout_name, scout_id)
        ...
        print(f"✗ export_node_global: {e}")
        return None


async def get_exported_nodes_for_display(
    current_guild_id: int,
    horizon_seconds: int = 4 * 3600,
) -> list[dict]:
    """Retorna nodes exportados de OUTRAS guilds que aparecem dentro das próximas
    `horizon_seconds` horas. Apenas canônicos (sem duplicatas). Nunca revela a guild
    de origem — o campo `scout_name` é omitido."""
    from datetime import datetime, timezone
    now_ts = int(datetime.now(timezone.utc).timestamp())
    max_ts = now_ts + horizon_seconds
    try:
        async with _global_db() as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                '''SELECT id, node_type, map_name, spawn_timestamp
                   FROM node_exports
                   WHERE source_guild_id != ?
                     AND canon_export_id IS NULL
                     AND spawn_timestamp > ? AND spawn_timestamp <= ?
                   ORDER BY spawn_timestamp ASC''',
                (current_guild_id, now_ts, max_ts),
            )
            rows = await cur.fetchall()
        return [dict(r) for r in rows]
    except Exception as e:
        print(f"✗ get_exported_nodes_for_display: {e}")
        return []
