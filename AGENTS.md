...
> **Produção:** se a tarefa envolve publicar, acessar a VPS, reiniciar serviço
> ou validar produção, leia primeiro `deploy/PRODUCAO.md`. Todo worker recebe
> este arquivo; não peça ao usuário o contexto de deploy que já está documentado.
...
- **Caminho escolhido: Microsoft Artifact Signing (Azure).** (Serviço renomeado de "Trusted Signing" pra "Artifact Signing" em meados de 2025 — resource provider `Microsoft.CodeSigning`, dlib `Azure.CodeSigning.Dlib.dll` e pacote NuGet `Microsoft.ArtifactSigning.Client` mantêm os nomes antigos.) Mais barato que certificado OV/EV tradicional, feito especificamente pra esse cenário (dev pequeno/individual), aceita verificação de pessoa física. **Restrição geográfica:** pessoa física só US/Canada; pessoa jurídica cobre US/Canada/EU/UK/AU/NZ/JP/KR/SG/CH/NO/IL.
- **Wiring já feito e verificado contra a doc atual (ago/2026)** (`tauri.conf.json` → `bundle.windows.signCommand` chama `scripts/sign-windows.ps1`, que roda `signtool /dlib /dmdf`). Sem essas env vars o script PULA a assinatura de propósito (exit 0) — build local/dev continua saindo sem assinar, igual sempre saiu, até o cadastro no Azure estar pronto. Não trave o dev loop tentando forçar assinatura sem conta configurada.
- **Flags do signtool verificados (ago/2026):** `signtool sign /v /fd SHA256 /tr "http://timestamp.acs.microsoft.com" /td SHA256 /dlib <x64\Azure.CodeSigning.Dlib.dll> /dmdf <metadata.json> <binário>`. O `/tr` + `/td` são **obrigatórios** — certificado Artifact Signing tem validade de **3 DIAS**, sem timestamp RFC 3161 a assinatura expira em 72h e o binário perde a confiança do Windows.
- **Dlib e signtool PRECISAM ser da mesma arquitetura** (x64+x64 ou x86+x86). O script busca signtool x64 no SDK; se usar x86, apontar `ZIGGS_TRUSTED_SIGNING_DLIB` pro `bin/x86/Azure.CodeSigning.Dlib.dll`.
- **Autenticação via DefaultAzureCredential** — `az login` (CLI) é o caminho mais simples. O dlib tenta EnvironmentCredential, ManagedIdentityCredential, AzureCliCredential, etc em sequência.
- **Endpoint DEVE bater com a região da conta** (tabela em learn.microsoft.com/azure/artifact-signing/quickstart) — endpoint errado = 403 Forbidden silencioso.
- **Ainda falta (só o dono consegue fazer — verificação de identidade):** criar o recurso Artifact Signing no Azure Portal (região East US/East US 2/West US 2/etc), passar pela verificação de identidade (Individual = passaporte/CNH via AU10TIX no browser; Organization = documento oficial), criar o certificate profile (Public Trust), dar a role "Artifact Signing Certificate Profile Signer" pra quem for assinar, instalar o Windows SDK (`signtool.exe` 10.0.2261.755+) + .NET 8 Runtime + Artifact Signing dlib (winget install `Microsoft.Azure.ArtifactSigningClientTools` instala tudo), e setar `ZIGGS_TRUSTED_SIGNING_ENDPOINT`/`_ACCOUNT`/`_PROFILE`/`_DLIB` no ambiente antes de rodar `npm run tauri build`. O `sign-windows.ps1` já faz busca de signtool e valida `Test-Path $dlib`.
...
**Presunção de preço (ago/2026):** quando `get_battle_prices` devolve 0 pra um item ilíquido (T8.4, artefatos raros, qualquer gear que não aparece nos 5 mercados nas últimas 8h), `get_battle_prices_with_presumption` em `app/services/prices.py` preenche o buraco em 3 passos, na ordem: (1) **qualidade** — varre cache `item_prices_latest` pelas qualidades 1-5 do MESMO item_id (q5 só se nenhuma 1-4); (2) **equivalência de tier** — gera cadeia T±k/E∓k (mesmo T+E, T∈[4,8], E∈[0,4]) e usa o 1º equivalente com preço real (tier MAIOR primeiro: 7.4→8.3); (3) **presunção de craft** — soma `(preço × qtd)` dos materiais da receita no `frontend/public/data/catalog.json` (carregado uma vez no startup via `_load_craft_catalog`, thread-safe) × `(1 − RRR bonus city sem focus)`. RRR bonus city = 33 pts → 33/133 ≈ 24.81% (fator 0.7519), sem focus porque focus é recurso finito (presumir que sempre tem focus é otimista demais). Artefato (`noReturn: true`) sem preço é **IGNORADO** (soma só materiais) — a NÃO SER que existam alternativas no catálogo: artefatos indexados por `(tier, slot category)` em `_ARTEFACTS_BY_TIER_AND_CATEGORY` (slot = weapon/offhand/helmet/armor/boots; qualquer faction AVALON/MORGANA/HELL/UNDEAD/KEEPER/ROYAL/CRYSTAL/FEY). Se um T4_ARTEFACT_2H_QUARTERSTAFF_AVALON falha, tenta outros T4_ARTEFACT_(2H|MAIN)_*_* (mesmo tier × arma), pegando o 1º com spot. Regra do usuário: **arma>arma, offhand>offhand, peito>peito** — nunca cross-categoria. Material principal (returnable) sem preço **ABORTA** a presunção daquele item (devolve 0 — não inventa número). Jornais (`*_JOURNAL_*` sem sufixo ou `_FULL`) fazem fallback pra versão `_EMPTY` correspondente (regra do usuário: não-vazio vale o preço do EMPTY) antes da equivalência. Sem recursão em presunção de materiais: se T7_METALBAR falha, o item que o usa não pressupõe — fica 0. Único caller vivo: `silver_dropped.py` (precificação de juicy kills). `get_battle_prices` original fica intacto — regear/highscores continuam usando preço REAL. Logs `silver_dropped` mostram `bases={...}` (conta `exact|quality|equivalent|presumed|missing`) pra auditoria. Catálogo desatualizado em prod = `catalog.json` deployado junto com frontend; se um patch do Albion trocar receitas, re-rodar `seed_craft_catalog_extras.py` + `npm run build` + restart do backend (catálogo lê em startup).`

**Testes (`backend/tests/`):** rodam com `PYTHONPATH=. python -m pytest backend/tests/ -q` (ou direto `python tests/test_X.py` pra um único). Maioria é sqlite-em-memória sem rede/Postgres; alguns exigem `aiosqlite` (`pip install aiosqlite`). Convenção: testes guardiões de regressão casam **texto do código** (source-inspection, `inspect.getsource`) em vez de comportamento — é barato e trava a estrutura, mas **quebra em refatoração** (renomear símbolo sem mudar comportamento falha o teste). Quem refatorar precisa atualizar os testes guardiões junto; rodar a suíte antes de commit/deploy é o único sinal de que isso foi feito. Ver `test_cold_load_grava_nucleo_antes_da_sync_de_kills` em `test_profile_refresh.py` (casa a ordem `upsert_player` ANTES de `sync_player_kills` nas 3 rotas que carregam perfil) — este padrão pegou o bug do "primeira carga não salva kills/deaths".
