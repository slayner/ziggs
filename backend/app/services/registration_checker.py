        ...
        async with make_client() as client:
            async with albion_scope(BOT_REGISTER):
                for rid, guild_id, region, albion_player_id, albion_player_name, is_ally, discord_user_id, role_id in reg_data:
                    ...
                    if reg is not None:
                        reg.active = False
                    # Persiste active=False e libera read tx antes do await
                    # (Discord API call em thread — read tx aberta impede
                    # wal_checkpoint).
                    db.commit()
                    if bot_token:
                        try:
                            await asyncio.to_thread(
                                ...
                                str(guild_id), str(discord_user_id), str(role_id), bot_token,
                            )
                        except Exception as e:
                            log.warning("registration_checker: falha ao remover cargo de %s: %s", discord_user_id, e)
                    motivo = "saiu da aliança permitida" if is_ally else "saiu da guilda Albion"
                    log.info("Registro de %s revogado (%s)", albion_player_name, motivo)
