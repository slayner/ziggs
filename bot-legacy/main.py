...
import database
from database import init_database, is_server_activated
import utils
...
async def on_ready():
    ...
    try:
        # SYNC_GUILD_ID (opcional): copia os comandos globais pra UMA guild e
        # sincroniza por lá — instantâneo. O sync GLOBAL propaga em até 1h.
        # Uso: dry-run local e primeiro boot (comandos caem na hora); depois
        # pode deixar vazio pro sync global normal.
        sgid = os.getenv('SYNC_GUILD_ID')
        if sgid:
            gobj = discord.Object(id=int(sgid))
            bot.tree.copy_global_to(guild=gobj)
            synced = await bot.tree.sync(guild=gobj)
            print(f'✓ {len(synced)} comando(s) sincronizado(s) na guild {sgid}')
        else:
...
async def on_app_command_error(interaction: discord.Interaction, error: discord.app_commands.AppCommandError):
    ...
    if interaction.guild and not await is_server_activated(interaction.guild.id):
        await _safe('❌ Este servidor não está ativado!\n'
