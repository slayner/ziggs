...
class EnergiaCog(commands.Cog, name="EnergiaCog"):
    ...
    # ==================================================================
    async def process_energy_log(self, text: str, guild_id: int | None = None) -> str:
        ...
        for ts, player, reason, amt in entries:
            if player not in nick_to_uid:
                # nick → usuário Discord vem do backend (bot NOVO, /register)
                # — o SQLite local não tem mais a tabela registrations.
                uid = (await http_client.lookup_user_by_nick(guild_id, player)
                       if guild_id else None)
                nick_to_uid[player] = uid
    ...
    async def energylog(self, ctx: commands.Context,
        ...
        report = await self.process_energy_log(log_text, ctx.guild.id if ctx.guild else None)
